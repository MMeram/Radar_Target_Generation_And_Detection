...
% Here we define the target's initial position and velocity. Note : Velocity
% remains contant
target_range = 50;
target_velocity = -30;
...
% Running the radar scenario over the time by having the constant velocity
% model
...
std_of_noise = zeros((2*Tr + 2*Gr + 1)*(2*Td + 2*Gd + 1) - (2*Gr+1)*(2*Gd+1) ,1);
...
for col = 1+Gr+Tr:(Nr/2)-(Gr+Tr)
    for row = 1+Gd+Td:(Nd)-(Gd+Td)
        ...
        for col2 = col-(Gr+Tr): col+(Gr+Tr)
            for row2 = row-(Gd+Td): row+(Gd+Td)
                ...
                if (abs(row-row2)> Gd || abs(col-col2)>Gr)
                    ...
                    noise_level = noise_level + db2pow(RDM(col2,row2));
                end
            end
        end
        ...
        % the offset value can also be identified as a function of standard
        % deviation. In our experiment 2*std(std_of_noise) has also worked
        ...
        % redefine the threashold with the offset
